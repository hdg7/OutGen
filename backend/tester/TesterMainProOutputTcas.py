...
class TesterMainProOutput(TesterMain):
...
#        insertTest(functionName,varVals,varTypes)
    ...
    def dumpTests(self,fileName="output.c"):
        ...
        for posi,inputVal in enumerate(self.inputs):
            self.insertTest(self.main,self.functionName,inputVal,self.varTypes,4,fileName,str(posi))
        self.createProgram()
        f=open(fileName,"w")
        for func in self.funcs:
            #print(generator.visit(func))
            if(isinstance(func,c_ast.FuncDef)):
                if (func.decl.name == "__bswap_32" or func.decl.name == "__bswap_64"):
                    continue
            if(isinstance(func, c_ast.Typedef)):
                continue
            if(isinstance(func, c_ast.Decl)):
                if(len(func.storage)>0 and func.storage[0] == 'extern'):
                    continue
            if not(isinstance(func,c_ast.FuncDef)):
                if(type(func.type)== c_ast.Enum or type(func.type)== c_ast.Struct or type(func.type)== c_ast.Union):
                    continue
#            func.show()
